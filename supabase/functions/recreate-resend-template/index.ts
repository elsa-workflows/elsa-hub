...
const OLD_TEMPLATE_ID = "b624d0a7-a9de-492a-8ab5-9707c193927e";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const deleteResponse = await fetch(`https://api.resend.com/templates/${OLD_TEMPLATE_ID}`, {
      method: "DELETE",
      headers: {
    ...
    if (!deleteResponse.ok) {
      ...
      // Continue even if delete fails (template might already be deleted)
    }

    // Step 2: Create new template with proper styling
    const templateHtml = `
...
<body style="margin: 0; padding: 0; background-color: #f4f4f5; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; -webkit-font-smoothing: antialiased;">
  ...
  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color: #f4f4f5;">
    ...
    const createResponse = await fetch("https://api.resend.com/templates", {
      method: "POST",
      headers: {
        Authorization: `Bearer ${RESEND_API_KEY}`,
    ...
    if (!createResponse.ok) {
      console.error("Resend API error:", createData);
      return new Response(
