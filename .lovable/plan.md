...
- Responsive grid: 1 col mobile, 2 col md, 2–3 col lg (keeps tiles large; a single provider spans the full container width).
- Each tile is a vertical card, variant `glass`, hairline border, generous padding.

Tile anatomy, top → bottom:

1. **Brand canvas** (full width of the card): a soft branded surface — subtle gradient using the provider's accent, with the icon mark rendered large and low-opacity (~6–8%) as a watermark, top-right. The full wordmark logo sits centered in the canvas (horizontally and vertically), sized to breathe (max-h ~80px, dark/light variant chosen via `useIsDark`).
...
- **New**: `src/components/enterprise/ProviderTile.tsx` — the tile component described above. Uses `useIsDark` to pick light/dark wordmark, imports the SVGs so Vite fingerprints them, and keeps the brand canvas full-width.
...
- **Edit**: `src/pages/enterprise/ExpertServicesProviders.tsx`:
  ...
  - Skeleton state: 2 skeleton tiles in the same grid.
